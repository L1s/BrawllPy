...
class Player(Actor):
	...
	def __init__(self):
		if type(self).spriteset is None:
			type(self).spriteset = Spriteset.fromfile("hero_test3")
	...
	def update_floor(self):
		...
		if not world.freezePlayers:	
			...
			if window.get_input(Input.A):
				# print("Jump key pressed from the floor")
				self.jump_counter = 0 ####
	...
	def update_air(self):
		...
		if not world.freezePlayers:	
			...
			if window.get_input(Input.A) and self.yspeed > 0:
				self.jump_counter += 1
				if self.jump_counter < 3:
					self.state = State.Idle
				else:
					self.state = State.Jump
				self.sprite.set_picture(13)
				player.set_jump()	
	...
	def update(self):
		...
		# print("self.medium = %d" % self.medium)
		# update immunity
		...
		####################
		# if self.y > oldy:	
		# 	self.animation.set_sprite_animation(0, seq_pack.sequences["seq_jump"], 0)
